...
#### Completion bounces finished handovers home — for review
Completing a task whose `return_when_done` is set (any task that arrived by
assignment and was not returned) moves the row back to the sender's board via
`PUT /api/tasks/:id { completed: true }` — **as a pending row with
`completed_by` stamped (the review state)**, dated the completer's today,
landing on a board the sender belongs to. The response's `owner_id` changes
(and `completed` comes back 0); clients should refetch when it does.
The sender then: completes it (`completed: true` — this is what completes a
linked subtask + dependents in their pane), reclaims it (`completed: false`
clears `completed_by` and un-assigns any linked step), or re-assigns it
(which re-arms `return_when_done` for another round).
